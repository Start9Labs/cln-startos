...
- **bitcoind is reached over the LXC bridge**, resolved by `bitcoindRpcBridge` in `startos/utils.ts` (the doctrine-v3 `bridgeAddress` helper) from bitcoin-core's exported `rpcHostId`/`rpcPort` (imported from `bitcoin-core-startos/startos/utils`). The config file model types `bitcoin-rpcconnect`/`bitcoin-rpcport` as optional (`.optional().catch(undefined)`) because the address is dynamic and absent until the binding resolves — never a fabricated placeholder.
